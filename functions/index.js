...
app.post("*", (req, res) => {
  ...
  /*
  // bracketjs
  if (req.headers.project === "bracketjs") {
  ...
  */
  ...
  // database
  if (path[1] === "database") return postdb({ req, res, db })
})
...
*/
  ...
  // database
  if (path[1] === "database") return deletedb({ req, res, db })
})
...
app.get("*", (req, res) => {
  ...
  // bracketjs
  if (req.headers.project === "bracketjs" || bracketDomains.includes(host)) {
  ...
  */
  ...
  // database
  if (path[1] === "database") return getdb({ req, res, db })
