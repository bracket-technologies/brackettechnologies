...
module.exports = async ({ req, res }) => {
  ...
  // resources
  if (path[1] === "resources") return getLocalFile({ req, res })
...
const checkHost = async ({ _window, global, host }) => {
  ...
  if (host.split(":")[1]) {
    ...
    // get host
    var { data: host } = await database({ _window, action: "search()", data: { db: bracketDB, collection: "host", findOne: { port: { inc: port } } } })
  ...
  // check host
    ...
    // get host
    var { data: host } = await database({ _window, action: "search()", data: { db: bracketDB, collection: "host", findOne: { localhost: host } } })
