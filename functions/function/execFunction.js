...
const execFunction = async ({ _window, req, res, id = generate() }) => {
  ...
  // interpret
  interpret({ _window, id, string: project.functions[func], req, res, _: data })
...
const interpret = ({ _window, id, string, req, res, _, __, ___ }) => {

  string = toCode({ _window, id, string: toCode({ _window, id, string }), start: "'", end: "'" })
  toParam({ _window, id, string, req, res, _, __, ___, mount: true })
}
