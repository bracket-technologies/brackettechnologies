...
const test = (component) => {
  ...
  // model
  component.featured = component.featured && "featured"
  component.model = component.model || component.featured || "classic"
  ...
  var {
    ...
    duplicatable, lang, unit, currency, google, key, minlength, container, generator,
  } = component
  ...
  // label
  if (!label) label = {}
  if (typeof label !== "object") label = { text: "Label Name" }
  if (!label.position) label.position = "outside"
  ...
  if (label.position === "outside") {
    ...
    return {
      id, Data, parent, derivations, path, __islabel__: true, preventDefault,
  ...
  if (model === 'featured' || password || clearable || removable || duplicatable || copyable || generator) {
    ...
    return {
      ...
      children: [{ // message
        ...
        controls: [...controls, {
          event: `clickfocus;keyfocus?if():[__labeled__]:[if():[!():${__labeled__}.contains():[clicked:()]]:[if():${duplicatable ? true : false}:[parent().click()]:[2ndChild().click()]]]:[if():[!():${id}.contains():[clicked:()]]:[click():[__droplistPositioner__:().del();]]]?!preventDefault`
        }, {
