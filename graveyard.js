...
const fs = require("fs")
const { getData, postData, deleteData } = require("./action/kernel")
...
var test0 = () => {
  ...
  var zzz = async () => {
    ...
    ["a1t7U0H8A8y877a0A1t9O05879", "01I7C0K8W8X8h720t1U9P123p6", "a2c7i1G4e4C800o5J6l377C5Z7"].map(async db => {
      var { data } = await getData({ _window: { global: { __queries__: {} } }, search: { db, collection: "view", doc: "route" } })
      data.children.map(child => child.view = child.view.replace("manifest:().route", "manifest:().action"))
      data.__props__.doc = "router"
      data.view = data.view.replaceAll("route", "router")
      var { data } = await postData({ _window: { global: { __queries__: {} } }, save: { db, collection: "view", doc: "router", data } })
      await deleteData({ _window: { global: { __queries__: {} } }, erase: { db, collection: "view", doc: "route" } })
      console.log(data);
    })
  }
  //zzz()